...
func TestFileFunctions(t *testing.T) {
	...
	require.Nil(t, err)
	gdte3, err := cli.Stat(ctx, &dssagrpc.Path{Path: ft})
	require.Nil(t, err)
	...
	require.Nil(t, err)
	gldte, err := cli.Stat(ctx, &dssagrpc.Path{Path: lt})
	require.Nil(t, err)
